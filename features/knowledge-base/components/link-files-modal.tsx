...
export function LinkFilesModal({
  ...
  const handleSubmit = async () => {
    const idsToLink =
      fileIds && fileIds.length > 0 ? fileIds : fileId ? [fileId] : [];
    if (!idsToLink.length) {
      ...
      <DialogContent className="sm:max-w-md">
        ...
        {fileIds && fileIds.length > 1 && (
          <div className="mb-2 text-xs text-muted-foreground">
            Linking <b>{fileIds.length}</b> files.
          </div>
        )}
                ...
                {isLoading ? (
                  ...
                  filteredKnowledgeBases.map((kb) => (
                    <CommandItem
                      ...
                      value={kb.name}
                      onSelect={() =>
                        toggleKnowledgeBase(kb.knowledgebase_id.toString())
                      }
                      className={cn(
                      ...
                      <div className="flex items-center">
                        <div
                          className={cn(
                            ...
                            selectedKbIds.includes(
                              kb.knowledgebase_id.toString()
                            )
        ...
        <DialogFooter>
          ...
          <Button
            ...
            disabled={isLinking || selectedKbIds.length === 0}
          >
