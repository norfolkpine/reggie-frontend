...
function useDataGrid<TData>({
  ...
  const columnSizeVars = React.useMemo(() => {
    ...
    return colSizes;
  }, [table.getState().columnSizingInfo, table.getState().columnSizing, defaultColumn.size, columns]);
  ...
  const onRowAdd = React.useCallback(
    async (event?: React.MouseEvent<HTMLDivElement>) => {
      ...
      // Force virtualizer to recalculate after row is added
      requestAnimationFrame(() => {
    ...
    [propsRef, onScrollToRow, readOnly, rowVirtualizer],
  );
  ...
  useIsomorphicLayoutEffect(() => {
    ...
    rowHeight,
    table.getRowModel().rows.length, // Re-measure when row count changes
