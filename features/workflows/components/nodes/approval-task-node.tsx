...
export function ApprovalTaskNode({ data, id }: NodeProps) {
  ...
  return (
    <div className="rounded-lg bg-pink-50 border-2 border-pink-300 shadow-sm w-[200px] relative group">
      <Handle
        type="target"
      ...
      <div className="p-3 space-y-2">
        <div className="flex items-center justify-between">
          <div className="bg-pink-100 rounded flex items-center justify-center gap-1 px-2 py-1">
            <UserCheck size={14} className="text-pink-700" />
            <span className='text-xs font-semibold text-pink-700'>Approval</span>
          </div>
          <Button
            onClick={onDelete}
        ...
        {data.config?.assignee && (
          <div className="text-xs text-pink-800 font-medium truncate">
            {data.config.assignee}
          </div>
        )}
      </div>
