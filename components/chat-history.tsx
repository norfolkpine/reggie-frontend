...
export function ChatHistory({ isMobile = false }: ChatHistoryProps) {
  ...
  const { chatSessions, isLoading: isLoadingChats, page: chatPage, deleteSession, renameSession } = useChatSessionContext();
  ...
  const handleChatRename = useCallback((sessionId: string, currentTitle: string) => {
  ...
  const handleChatDelete = useCallback(async (sessionId: string) => {
        ...
        <div key={section.title} className="mb-4">
          ...
          {section.expanded && (
            <div className="space-y-2 mt-2">
              {(isLoadingChats && chatPage === 1 ? [] : section.items).map((item, itemIndex) => (
                <div
                  key={`chat ${item.id} ${itemIndex}`}
                  className={cn(
                    "rounded-lg p-2 transition-colors duration-150 group relative",
                    (selectedSessionId === item.id || editingSessionId === item.id) ? "bg-sidebar-accent" : "",
