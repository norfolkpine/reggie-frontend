...
export const getKnowledgeBasePdfUrls = async (page: number = 1) => {
  const response = await api.get('/opie/api/v1/knowledge-base/pdf-urls/', {
...
export const getKnowledgeBasePdfUrl = async (id: string) => {
  const response = await api.get(`/opie/api/v1/knowledge-base/pdf-urls/${id}/`);
...
export const createKnowledgeBasePdfUrl = async (
  ...
  const response = await api.post('/opie/api/v1/knowledge-base/pdf-urls/', pdfUrl);
  return response as KnowledgeBasePdfURL;
};
...
export const updateKnowledgeBasePdfUrl = async (
  ...
  const response = await api.put(`/opie/api/v1/knowledge-base/pdf-urls/${id}/`, pdfUrl);
  return response as KnowledgeBasePdfURL;
};

export const patchKnowledgeBasePdfUrl = async (id: string, pdfUrl: PatchedKnowledgeBasePdfURL) => {
  const response = await api.patch(`/opie/api/v1/knowledge-base/pdf-urls/${id}/`, pdfUrl);
  return response as KnowledgeBasePdfURL;
};

export const deleteKnowledgeBasePdfUrl = async (id: string) => {
  await api.delete(`/opie/api/v1/knowledge-base/pdf-urls/${id}/`);
}; 